...
open class AnalogJoystick: SKNode {
    ...
    init(substrate: AnalogJoystickSubstrate, stick: AnalogJoystickStick) {
        ...
        var timer = Timer.scheduledTimer(timeInterval: 0.3, target: self, selector: #selector(self.vibrate), userInfo: nil, repeats: true);
    }
    ...
    func vibrate() {
        if let touch = touch {
            ...
            else if touch.force > 6.2 {
                let generator = UIImpactFeedbackGenerator(style: .medium)
                generator.prepare()
                generator.impactOccurred()
            }
            else if touch.force > 6.0 {
