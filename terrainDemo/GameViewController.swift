...
class GameViewController: NSViewController, SCNSceneRendererDelegate, GameViewDelegate, TileGeneratorDelegate {
    ...
    override func awakeFromNib(){
        ...
        cameraNode.position = SCNVector3Make(1500,1500,1500)
       ...
       // floor.reflectionFalloffStart = 0.0
        ...
        lightNode.position = SCNVector3(x: 0, y: 1000, z: -180)
    ...
    func renderer(_ renderer: SCNSceneRenderer, updateAtTime time: TimeInterval) {
        ...
        if(isMoving != 0){
            cameraNode.position.x -= b.x*25*CGFloat(isMoving)
            cameraNode.position.z -= b.z*25*CGFloat(isMoving)
        }
