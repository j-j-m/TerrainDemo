...
extension SCNMatrix4: Matrix {
    public init(_ contents: [[CGFloat]]) {
    ...
    public var CGFloatArray: [[CGFloat]] {
        return [[m11, m12, m13, m14],
            [m21, m22, m23, m24],
            [m31, m32, m33, m34],
            [m41, m42, m43, m44]]
...
public func *(left: SCNMatrix4, right: CGFloat) -> SCNMatrix4 {
    return SCNMatrix4(
...
public func *(left: CGFloat, right: SCNMatrix4) -> SCNMatrix4 {
    return right * left
}
...
//    sgetri_(&nc, &(grid), &nc, &ipiv, &work, &lwork, &error)
    ...
    assert(error == 0, "MatrixCGFloat not invertible")
